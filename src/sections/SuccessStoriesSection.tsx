...
const projects: ProjectData[] = [
  {
    ...
    solution:
      'Complete website modernization with commission-free ordering, local SEO across 6 locations, and automated customer engagement pipeline.',
    impact:
  ...
  },
  {
    ...
    solution:
      'Automated lead capture, instant SMS follow-up, integrated CRM visibility, and AI-driven qualification scoring.',
    impact:
  ...
  },
  {
    ...
    duration: '10 Weeks',
    problem:
      'No real-time visibility into inventory levels, order status, or fulfillment bottlenecks across 3 warehouse locations.',
    solution:
      'Centralized reporting dashboard with live inventory tracking, automated reorder alerts, and fulfillment pipeline analytics.',
    impact:
...
function AnimatedMetric({ val, label }: { val: string; label: string }) {
  const numVal = parseInt(val.replace(/[^0-9]/g, ''));
  const prefix = val.startsWith('+') ? '+' : val.startsWith('-') ? '-' : '';
  const suffix = val.endsWith('x') ? 'x' : val.endsWith('%') ? '%' : '';
  const { count, ref } = useAnimatedCounter(numVal, 2);
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
        <div className="lg:col-span-5 flex flex-col justify-start">
          ...
          <div className="flex flex-col gap-4 border-y border-border py-6 mb-6">
            {project.metrics.map((metric) => (
              <AnimatedMetric key={metric.label} val={metric.val} label={metric.label} />
            ))}
          </div>
        ...
        <div className="lg:col-span-7 space-y-8 flex flex-col justify-start pt-2 lg:pt-0">
          <div>
            <h4 className="text-[11px] uppercase tracking-wider text-accent font-extrabold mb-2.5">Challenge</h4>
            <p className="text-sm md:text-[15px] text-muted-foreground leading-relaxed font-sans">{project.problem}</p>
          </div>
          <div className="border-t border-border pt-6">
            <h4 className="text-[11px] uppercase tracking-wider text-accent font-extrabold mb-2.5">Solution</h4>
            <p className="text-sm md:text-[15px] text-muted-foreground leading-relaxed font-sans">{project.solution}</p>
          </div>
          <div className="border-t border-border pt-6">
            <h4 className="text-[11px] uppercase tracking-wider text-accent font-extrabold mb-2.5">Outcome</h4>
            <p className="text-sm md:text-[15px] font-bold text-foreground leading-relaxed font-sans">{project.impact}</p>
          </div>
        </div>
      </div>
    </motion.div>
  ...
  return (
    <SectionContainer id="success-stories" background="white">
      ...
      <p className="text-lg text-muted-foreground text-center max-w-2xl mx-auto mt-4">
        Real results for real businesses. Measurable outcomes across every engagement.
      </p>
